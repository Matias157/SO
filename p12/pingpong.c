...
int cont = 1, tasksSem = 0, tasksBar = 0, mainflag = 0;
...
int mqueue_create (mqueue_t *queue, int max, int size){
	...
	queue->insert_pos = 0; //posição inicial de envio
	queue->remove_pos = 0; //posição inicial de recebimento
	...
	queue->message_content = (char **)malloc(max*sizeof(char*)); //aloca um vetor de strings com max posições
	int i = 0;
	while(i < max){
		queue->message_content[i] = (char*)malloc(size*sizeof(char)); //para cada posição alocamos uma string tamanho maximo de size
		i++;
	}
...
int mqueue_send (mqueue_t *queue, void *msg){
	...
	char *csrc = (char *)msg; //typevast para msg ser interpretada como char
  	char *cdest = (char *)queue->message_content[queue->insert_pos]; //desnecessário, mas deixei pra ficar bonito
	for(int i = 0; i < queue->messages_size; i++) 
    	cdest[i] = csrc[i]; //coloca a mensagem bit a bit na posição livre de envio

	queue->insert_pos++; //informa que uma posição de envio esta ocupada
	if(queue->insert_pos > queue->max_messages - 1) //caso todas as posições estejam ocupadas
		queue->insert_pos = 0; //coloca na posição 0

	sem_up(&queue->receive); //coloca o semaforo de recebimento em up
...
int mqueue_recv (mqueue_t *queue, void *msg){
	...
	char *csrc = (char *)queue->message_content[queue->remove_pos]; //desnecessário, mas deixei pra ficar bonito
  	char *cdest = (char *)msg; //typevast para msg ser interpretada como char
	for(int i = 0; i < queue->messages_size; i++) 
    	cdest[i] = csrc[i]; //coloca a informação da posição livre bit a bit na mensagem

	free(queue->message_content[queue->remove_pos]); //desaloca o vetor de string de recebimento

	queue->remove_pos++; //informa que uma posição de envio esta ocupada
	if(queue->remove_pos > queue->max_messages - 1) //caso todas as posições estejam ocupadas
		queue->remove_pos = 0; //coloca na posição 0

	sem_up(&queue->send); //coloca o semaforo de envio em up
